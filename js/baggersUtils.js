...
function displayContactModalWindowMail() {
    $('.invite-me').click(function (e) {
        ...
        $('#contactModal').find('#contact-to').val(bagger.contacts.mail);
...
function transformTitleToPermalink(title) {
    return removeDiacritics(title).replace(/ /g, '-').replace(/[:?,]/g, '').toLowerCase();
}
